...
$prefDoctorSpec = $doctor ['specialist'];
...
$queryDoctors = "SELECT person.email, person.fname, person.lname, doctor.specialist, doctor.doctorid, ROUND(AVG(doctorreview.rating),2) FROM person INNER JOIN doctor on person.email=doctor.email inner join appointment on doctor.doctorid=appointment.doctorid inner join doctorreview on appointment.appid = doctorreview.appid WHERE person.role='doctor' GROUP BY appointment.doctorid";
$getDoctors = mysqli_query ( $conn, $queryDoctors );
$doctors = mysqli_fetch_all ( $getDoctors );

$queryHistory = "SELECT appointment.appid, doctor.specialist, person.fname, person.lname, appointment.date, appointment.prescription FROM appointment inner join doctor on appointment.doctorid = doctor.doctorid inner JOIN person on doctor.email = person.email where patientid=$patientID and date <= CURDATE() ORDER BY appointment.date DESC";
...
$queryUpcApp = "SELECT appointment.appid, doctor.specialist, person.fname, person.lname, appointment.date, TIME_FORMAT(appointment.time, '%H:%i'), appointment.doctorid FROM appointment inner join doctor on appointment.doctorid = doctor.doctorid inner JOIN person on doctor.email = person.email where patientid=$patientID and date > CURDATE() ORDER BY appointment.date, appointment.time";
			...
			<div class="content-grids">
				...
				<div class="col-md-4 testi">
					...
					<div id="top" class="callbacks_container">
						...
						<span id="ajaxMessage"></span> <br/>
						<select id="chooseDr" class="btn1" style="display: none">
							<?php
							if ($prefDoctor == null) {
								echo "<option></option>";
							}
						...
						<script type="text/javascript">
						$("#changeDr").click(function(){$("#chooseDr").show();});
						$("#chooseDr").change(function(){$("#ajaxMessage").text("").show();
